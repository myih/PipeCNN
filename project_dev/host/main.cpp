...
#define IMAGE_FILE_SIZE   (19*19*32*64)
//#define WEIGHTS_FILE_SIZE 60965224 //fc8-1000
#define WEIGHTS_FILE_SIZE (3*3*32*64)  //fc8-1024
#define LAYER_NUM         1
#define CONV_NUM          1
const char *weight_file_path = "../data/test/rawweight.dat";
const char *input_file_path = "../data/test/rawimage.dat";
...
conv_x, conv_y, conv_z, conv_stride, conv_padding, conv_split, dilate_rate, conv_relu, //Conv Parameters // conv_z=weight_m
	...
	for(unsigned i = 0; i < num_devices; ++i) {
		...
		for(unsigned char j = 0; j < LAYER_NUM; ++j){
			...
			if(layer_config[j][conv_x]==1){
				...
				conv_win_size_dim1  = (layer_config[j][weight_w]-1) * layer_config[j][dilate_rate] + 1 +(CONV_GP_SIZE_X-1)*layer_config[j][conv_stride];
				if(layer_config[j][conv_x]%CONV_GP_SIZE_X==0)
					conv_group_rem_dim1   = CONV_GP_SIZE_X*((layer_config[j][weight_w]-1) * layer_config[j][dilate_rate] + 1) ;
				else
					conv_group_rem_dim1   = layer_config[j][conv_x]%CONV_GP_SIZE_X*((layer_config[j][weight_w]-1) * layer_config[j][dilate_rate] + 1); 
			}
			conv_win_size_dim2    = layer_config[j][weight_h];//(layer_config[j][weight_h] - 1) * layer_config[j][dilate_rate] + 1;
			conv_group_rem_dim2   = layer_config[j][weight_h];//(layer_config[j][weight_h] - 1) * layer_config[j][dilate_rate] + 1;

			conv_win_size_dim1x2x3  = conv_win_size_dim1*conv_win_size_dim2*layer_config[j][weight_n];
			conv_group_rem_dim1x2x3 = conv_group_rem_dim1*conv_group_rem_dim2*layer_config[j][weight_n]; printf("conv_group_rem_dim1x2x3=%d\n",conv_group_rem_dim1x2x3);
			...
			status = clSetKernelArg(knl_memRd[i], argi++, sizeof(cl_uchar), &layer_config[j][dilate_rate]);
			checkError(status, "Failed to set argument %d of kernel memRd", argi - 1);
...
void loadImageToBuffer(int num)
{
    ...
    if(bin_file_r.is_open())
    {
    	...
    	printf("image:\n");
    	printf("n=%d\n",layer_config_original[0][data_n]);
    	for(int n=0; n<layer_config_original[0][data_n]; n++)
    	for(int i=0; i<layer_config[0][data_h]; i++)
    		{
    		for(int j=0; j<layer_config[0][data_w]; j++)
    			printf("%d ",  image[n*layer_config[0][data_h]*layer_config[0][data_w]+i*layer_config[0][data_w]+j]);
    		printf("\n");
    		}
    	/*int *tmp;
    	tmp = (int *)alignedMalloc(sizeof(int)*IMAGE_FILE_SIZE, DMA_ALIGNMENT);
    	bin_file_r.read((char *)tmp, sizeof(int)*IMAGE_FILE_SIZE);
    	for(int i:IMAGE_FILE_SIZE) image[i]=(char*)tmp[i];*/
    	//for(int i=0;i<IMAGE_FILE_SIZE; i++) {bin_file_r>>image[i]; printf("image[%d]=%d", i, image[i]);}
...
int prepare()
{
	...
	for(unsigned ll=0; ll<LAYER_NUM; ll++){
		...
		if((layer_config_original[ll][conv_x]!=(layer_config_original[ll][data_w]-((layer_config_original[ll][weight_w]-1)*layer_config_original[ll][dilate_rate]+1)+2*layer_config_original[ll][conv_padding])/layer_config_original[ll][conv_stride]+1)
			|| (layer_config_original[ll][conv_y]!=(layer_config_original[ll][data_h]-((layer_config_original[ll][weight_h]-1)*layer_config_original[ll][dilate_rate]+1)+2*layer_config_original[ll][conv_padding])/layer_config_original[ll][conv_stride]+1)
			...
			printf("layer_config_original[ll][conv_x]=%d, !=%d\n",layer_config_original[ll][conv_x],(layer_config_original[ll][data_w]-((layer_config_original[ll][weight_w]-1)*layer_config_original[ll][dilate_rate]+1)+2*layer_config_original[ll][conv_padding])/layer_config_original[ll][conv_stride]+1);
			printf("part=%d",(layer_config_original[ll][data_h]-((layer_config_original[ll][weight_h]-1)*layer_config_original[ll][dilate_rate]+1)+2*layer_config_original[ll][conv_padding]));
			printf("layer_config_original[ll][data_h]=%d, layer_config_original[ll][weight_h]=%d, layer_config_original[ll][dilate_rate]=%d, layer_config_original[ll][conv_padding])=%d, layer_config_original[ll][conv_stride]=%d, ",
			layer_config_original[ll][data_h], layer_config_original[ll][weight_h], layer_config_original[ll][dilate_rate], layer_config_original[ll][conv_padding], layer_config_original[ll][conv_stride]);
			return 1;
		}
    ...
    if(bin_file_r.is_open())
    {
		...
		printf("weights: \n");
		printf("n=%d, m=%d",layer_config_original[0][weight_n], layer_config[0][weight_m]);
		for(int n=0; n<layer_config_original[0][weight_n]; n++)
		for(int m=0; m<layer_config[0][weight_m]; m++)
    	for(int i=0; i<layer_config[0][weight_h]; i++)
    		{
    		for(int j=0; j<layer_config[0][weight_w]; j++)
    			printf("%d ", weights[n*layer_config[0][weight_m]*layer_config[0][weight_w]*layer_config[0][weight_h]+m*layer_config[0][weight_w]*layer_config[0][weight_h]+i*layer_config[0][weight_w]+j]);
    		printf("\n");
    	}
