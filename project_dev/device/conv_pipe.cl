...
void memRead(
			// Params Ports
			uchar  data_dim1, // input data dimension 
			uchar  data_dim2,
			ushort data_dim1xdim2,
			uchar  weight_dim1, // weight dimension
			uchar  weight_dim2,
			ushort weight_dim3,
			ushort weight_dim4_div_lane, // avoid generating divider // weight_m(= conv_z) divided by lane // output_z per lane
			uchar  weight_dim1x2,
			uint   weight_dim1x2x3,
			uchar  conv_x,		// output dimension	
			//uchar  conv_y,           // not used in this version
			uchar  stride,
			uchar  padding,
			uchar  split,
			uchar  dilate_rate, // for dilate convolution
			uchar  group_num_x,		// number of groups doing sliding window
			uchar  group_num_y,
			uchar  group_rem_size_x, 	// remaining parts outside sliding window
			//uchar  group_rem_size_y, // not used in this version
			uint   group_rem_size_xyz,
			uchar  win_size_x,		// window size //weight_w +(CONV_GP_SIZE_X-1)*stride ////////////////////for dilate_conv something should change here?
			uchar  win_size_y, 		// window hight size whould change with different dilate rate
			uint   win_size_xyz,  
			// Data Ports
...

{
	...
	for(unsigned short win_itm_z=0; win_itm_z<weight_dim3/VEC_SIZE; win_itm_z++){
		for(unsigned char  win_itm_y=0; win_itm_y<win_size_y; win_itm_y++){
			...
			feature_idx_dim2 = win_itm_y*dilate_rate;//***********************
			...
			if((feature_idx_dim1>=padding && feature_idx_dim1<data_dim1+padding) && (feature_idx_dim2>=padding && feature_idx_dim2<data_dim2+padding)){
				...
				//printf("win_itm_xyz=z win_itm_x=%d win_itm_y=%d pos=(%d,%d,%d) data=%d\n",  win_itm_x, win_itm_y, feature_idx_dim1, feature_idx_dim2, feature_idx_dim3, data_vec.data[0]);
			}
			else{
				#pragma unroll
				for(unsigned char vv=0; vv<VEC_SIZE; vv++){
					data_vec.data[vv] = CZERO; //0
					//if(vv==0)printf("win_itm_xyz=z win_itm_x=%d win_itm_y=%d pos=(%d,%d,%d) data=%d\n",  win_itm_x, win_itm_y, feature_idx_dim1, feature_idx_dim2, feature_idx_dim3, data_vec.data[0]);
				}
			}
...
#ifdef test ////////////////////for input width smaller then 7(window size)////////////
	if(group_num_x==1) gp_num_y_winbuf = 1;
	else gp_num_y_winbuf = 0;
#else
	gp_num_y_winbuf = 0;
#endif
	out_idx_z_winbuf = 0;
	...
	Group:for(unsigned int out_idx_xyz=0; out_idx_xyz<(weight_dim4_div_lane*group_num_y*group_num_x); out_idx_xyz++){ /////// might be doing one extra iteration here... (the last buffer was not used) should check ///////

		// special case when split==1, the output feature maps depend on only half the input feature maps
		if(split==0)
			data_offset = 0;
		else if(out_idx_z_winbuf<(weight_dim4_div_lane>>1)) // the upper half of the output feature maps depend on the upper half of the input
			data_offset = 0;
		else
			data_offset = weight_dim3/VEC_SIZE;	// the lower half of the output feature maps depend on the lower half of the input
	
				flag = out_idx_xyz & 0x01; //ping-pong flag, starts with 0
				...
				if(gp_num_x==group_num_x-1)	{// last group or only group
					item_loop_bound = win_size_x>=group_rem_size_x?(win_size_xyz/VEC_SIZE):(group_rem_size_xyz/VEC_SIZE); 
					//printf("group%d size=%d\n",gp_num_x, item_loop_bound);
					//printf("win_size_x=%d, group_rem_size_x=%d, win_size_xyz=%d, group_rem_size_xyz=%d, item_loop_bound=%d\n", win_size_x, group_rem_size_x, win_size_xyz, group_rem_size_xyz, item_loop_bound);
					} 																									// last group deal with the remining convolution
																														// win_buffer can be use for the remaining convolution if it's 
																														// large enough, otherwise use precompute parameter 
				else{
					item_loop_bound = (weight_dim1x2x3*CONV_GP_SIZE_Y*CONV_GP_SIZE_X/VEC_SIZE);//(((weight_dim1-1)*dilate_rate+1) * ((weight_dim2-1)*dilate_rate+1) * weight_dim3*CONV_GP_SIZE_Y*CONV_GP_SIZE_X/VEC_SIZE);	// number of items to be computed within one sliding window group
					//printf("group%d size=%d\n",gp_num_x, item_loop_bound);
				}
				//printf("group%d size=%d\n",gp_num_x, item_loop_bound);
				//if(out_idx_xyz==0) printf("CONV_GP_SIZE_X=%d\n", CONV_GP_SIZE_X);
				//if(out_idx_xyz==0) printf("item_loop_bound=%d\n", item_loop_bound);
				//printf("out_idx_xyz=%d item_loop_bound=%d\n",out_idx_xyz, item_loop_bound);
				#pragma ivdep array(win_buffer) // ignore dependency of win_buffer between loops 
				#pragma ivdep array(weight_buffer) // ignore dependency of weight_buffer between loops 
				Item:for(unsigned int  win_itm_xyz=0; win_itm_xyz<item_loop_bound; win_itm_xyz++){

							// Winbuffer loading operations // Prefetch
							if(win_itm_z<weight_dim3/VEC_SIZE){ // until the buffer is fully loaded
															// gp_num_x_winbuf starts from 1: load for the next loop
								feature_idx_dim1 = win_itm_x+gp_num_x_winbuf*CONV_GP_SIZE_X*stride; // window item position + window group offset // not dimension
								feature_idx_dim2 = win_itm_y*dilate_rate+gp_num_y_winbuf*CONV_GP_SIZE_Y*stride; //******************************************************
								feature_idx_dim3 = win_itm_z;
								...
								if((feature_idx_dim1>=padding && feature_idx_dim1<data_dim1+padding) && (feature_idx_dim2>=padding && feature_idx_dim2<data_dim2+padding)){
									...
									last group will cause the loading and sending data to be inbalanced 
									...
									*/
									...
									{
										data_vec = bottom[data_offset*data_dim1xdim2 + feature_idx_dim3*data_dim1xdim2 + (feature_idx_dim2-padding)*data_dim1 + (feature_idx_dim1-padding)];
										//if(gp_num_x==group_num_x-1)printf("win_itm_xyz=%d win_itm_x=%d gp_num_y_winbuf=%d pos=(%d,%d,%d) data=%d\n", win_itm_xyz, win_itm_x, gp_num_y_winbuf, feature_idx_dim1, feature_idx_dim2, feature_idx_dim3, data_vec.data[0]);
									...
									for(unsigned char vv=0; vv<VEC_SIZE; vv++){
										...
										//if(gp_num_x==group_num_x-1)if(vv==0)printf("win_itm_xyz=%d win_itm_x=%d gp_num_y_winbuf=%d pos=(%d,%d,%d) data=%d\n", win_itm_xyz, win_itm_x, gp_num_y_winbuf, feature_idx_dim1, feature_idx_dim2, feature_idx_dim3, data_vec.data[0]);
									}
								}
								
								//////////////////if(win_itm_xyz<27) do extra work when win_itm_xyz is larger then the buffer size/////////////////////need to find out why///////////
								win_buffer[(~flag)&0x01][win_itm_z*win_size_y*win_size_x + win_itm_y*win_size_x + win_itm_x] = data_vec;

								// used as loop counters
								if((win_itm_z==weight_dim3/VEC_SIZE-1) && (win_itm_y==win_size_y-1) && (win_itm_x==win_size_x-1)) // reached the end
									win_itm_z = 0;
							...
							if(gp_item_idx_x==0){ // first window position in the group
								// out_idx_z: global group counter // output_idx_dim1/2/3: item loop counter
							...
							if(gp_num_x*CONV_GP_SIZE_X+gp_item_idx_x<conv_x){
								...
								if(output_idx_dim1==0 && output_idx_dim2==0 && output_idx_dim3==0){ // only one bias for one feature map
								...
								// data
								// buffer data loaded from privious group loop
								data_vec = win_buffer[flag][output_idx_dim3*win_size_y*win_size_x + output_idx_dim2*win_size_x + (output_idx_dim1*dilate_rate+gp_item_idx_x*stride)];
								//printf("flag=%d, data=%f, (%d, %d), gp_item_idx_x=%d pos=%d \n", flag, (float)data_vec.data[0], output_idx_dim1, output_idx_dim2, gp_item_idx_x-'0'+48,
									//output_idx_dim3*win_size_y*win_size_x + output_idx_dim2*win_size_x*dilate_rate + (output_idx_dim1*dilate_rate+gp_item_idx_x*stride));
								#pragma unroll
								for(unsigned char ll=0; ll<LANE_NUM; ll++){
									data_ch_vec.lane[ll] = data_vec;
								}
								write_channel_altera(data_ch, data_ch_vec);	
								//printf("data=%f, win_itm_xyz=%d,  gp_item_idx_x=%d\n",(float)data_vec.data[0], win_itm_xyz, gp_item_idx_x-'0'+48);
								...
								//if(gp_num_x==group_num_x-1 && gp_num_y==0 && out_idx_z==0){
									printf("gp_num_y_winbuf=%d, work-item x=%d, y=%d, z=%d, offset=%d, write data in channel 0=%f\n", gp_num_y_winbuf, output_idx_dim1, output_idx_dim2, output_idx_dim3, data_offset, (float)data_ch_vec.lane[0].data[0]);
...
void coreConv(
			// Params Ports
			uint  output_num,
			uint  conv_loop_cnt, // weight_w*weight_h*weight_n/VEC_SIZE
			...
			)
{
	...
	for(unsigned int k=0; k<output_num; k++){
		...
		for(int j=0; j<conv_loop_cnt; j++){// one 3d weight convolution

			mac_data = read_channel_altera(data_ch); //printf("in=%6.f\n", (float)mac_data.lane[0].data[0]);
			mac_weight = read_channel_altera(weight_ch);

			#pragma unroll
			for(unsigned char ll=0; ll<LANE_NUM; ll++){
				//previous stored data + mult-add(data, weight)
				...
				for(unsigned int p=PIPE_DEPTH-1; p>0; p-- ){ 
					accum_piped[ll][p] = MASK_ACCUM & accum_piped[ll][p-1]; // shift data //PIPE_DEPTH = 6
				}
		...
		for(unsigned char ll=0; ll<LANE_NUM; ll++){
			...
			if(conv_with_rnd_bit[ll]>=256) // overflow
			...
			#endif
				if(printcounter<outsize-1)//layer_config[0][10]
				{
					printf("%6.f ",(float)conv_out[ll]);
					printcounter++;
				}
				else // ==end
				{
